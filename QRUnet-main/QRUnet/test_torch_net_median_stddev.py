...
def reconstruct_slice(fnet, subsampled_slice, subsampled_slice_k, substep,
    ...
    fnet_input = np.expand_dims(fnet_input, 0)
    test_loader = torch.utils.data.DataLoader(fnet_input,
                                              batch_size=1,
                                              shuffle=False)
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(test_loader):
            if torch.cuda.is_available():
                in_data = data[0:1].cuda()
                recon_batch = fnet(in_data)
                    #loss = qr_loss(out_data, recon_batch, q=CONST_Q)
                    #test_loss += loss.item()                                          

    fnet_output = np.squeeze(recon_batch.cpu().numpy())
...
def main():
    ...
    parser.add_argument(
        ...
        default='/ImagePTE1/akrami/MRI_Recon_results/torch/qr_0.5fnet.pt',
        help="The path to a trained FNet")
    parser.add_argument(
        '-n_ql', '--net_path_ql', type=str,
        default='/ImagePTE1/akrami/MRI_Recon_results/torch/qr_0.85fnet.pt',
        help="The path to a trained FNet")
    parser.add_argument(
        '-n_qu', '--net_path_qu', type=str,
        default='/ImagePTE1/akrami/MRI_Recon_results/torch/qr_0.15fnet.pt',
        help="The path to a trained FNet")
    parser.add_argument(
        '-d',
        '--data_path',
    ...
    parser.add_argument(
        ...
        '--results_dir',
        type=str,
        default='/ImagePTE1/akrami/MRI_Recon_results',
